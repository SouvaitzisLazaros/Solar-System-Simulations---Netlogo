...
to setup
    ...
    crt 1
    ;in the study of asteroids we will just use 6 planets!!!

    create-planets 6                ;turtle 9 is the comet --> might be affected by other planets (Sun, Jupiter, Saturn) if it gets close enough
    create-asteroids Number_of_Asteroids
    ...
    ask asteroids [
      ...
      set color gray
      ;mass - slider
      set mass random-float MaxMass
      ;radius range
      let lowR 0.85
      let highR 1.5
      let range1 highR - lowR
      set avg-radius lowR + random-float range1
      ;size
      let lowS 500
      let highS 1200
      let rangeS highS - lowS
      set size (log (lowS + random-float rangeS) 10) / 10
      ;random initial velocities
      let lowV 0
      ;let highV 10 ^ (-5)
      let rangeV MaxVelocity - lowV
      set Vx lowV + random-float rangeV
      set Vy lowV + random-float rangeV
      ]
   ...
   ask asteroids [
    ...
    ask asteroids [set radius sqrt(x ^ 2 + y ^ 2) ]
    ...
    ask planets [
        ...
        ;compute acceleration in x and y directions due to gravities of other planets
                ...
                ask planets [
                    ...
                    ask asteroids with [who != [who] of myself] [
...
to IsRuning
    ...
    ask asteroids [
                ...
                set Vy Vy + aY * time
                set sqVy Vy ^ 2
                set V sqrt ( sqVx + sqVy )
                ]


  ;caclulate the average velocity of astetroids
  set avg_asteroid_v mean [V] of asteroids
...
to PlotSetup
  ...
  ; Plot 1
  set-current-plot "Incoming/Outgoing Asteroids"
  set-current-plot-pen "incoming"
  plot count asteroids with [radius < [radius] of Mars]
  set-current-plot-pen "outgoing"
  plot count asteroids with [radius > [radius] of Jupiter]

  ; Plot 2
  set-current-plot "Average Asteroid Velocity "
  set-current-plot-pen "v > V_avg"
  plot count asteroids with [sqrt (Vx ^ 2 + Vy ^ 2 ) > avg_asteroid_v ]
  set-current-plot-pen "v < V_avg"
  plot count asteroids with [sqrt (Vx ^ 2 + Vy ^ 2 ) < avg_asteroid_v ]

  ;----------------------------------------------------------------
  ;some examples for solar system study
...
55
Incoming_Asteroids
count asteroids with [radius < [radius] of Mars]
17
1
11
...
108
Outgoing_Asteroids
count asteroids with [radius > [radius] of Jupiter]
17
1
11
...
"" ""
PENS
"Incoming" 1.0 0 -2674135 true "" "plot count asteroids with [radius < [radius] of Mars]"
"Outgoing" 1.0 0 -14070903 true "" "plot count asteroids with [radius > [radius] of Jupiter]"
...
1348
296
High Velocity Asteroids
count asteroids with [sqrt (Vx ^ 2 + Vy ^ 2 ) > avg_asteroid_v ]
17
1
11
...
"" ""
PENS
"v > V_avg" 1.0 0 -2674135 true "" "plot count asteroids with [sqrt (Vx ^ 2 + Vy ^ 2 ) > avg_asteroid_v ]"
"v < V_avg" 1.0 0 -14070903 true "" "plot count asteroids with [sqrt (Vx ^ 2 + Vy ^ 2 ) < avg_asteroid_v ]"
...
count asteroids with [(radius - 1 / 3) < ([radius] of Mars - [radius] of Earth) / 2]
17
1
11
...
count asteroids with [(([radius] of Jupiter / radius) ^ 3) mod 2 = 0]
17
1
11
...
586
NIL
sqrt(([Vx] of Jupiter) ^ 2 + ([Vy] of Jupiter) ^ 2)
17
1
11
